...
from .token_manager import TokenHandler, TokenAwareRetriever
...
class RAGEngine:
    ...
    def __init__(self, embedding_model: Embeddings, config: Dict[str, Any]):
        ...
        self.k = retrieval_config.get('k', 8)
    ...
    def setup_qa_chain(self):
        ...
        **DO NOT HALLUCINATE OR MAKE UP INFORMATION.**
        ...
        # Create base retriever and wrap it with token-aware filtering
        base_retriever = self.vector_db.as_retriever(search_kwargs={"k": self.k})
        token_aware_retriever = TokenAwareRetriever(base_retriever, self.token_handler)
        
        # Final QA chain
        self.qa_chain = RetrievalQA(
            combine_documents_chain=map_reduce_chain,
            retriever=token_aware_retriever,
    ...
    def ask(self, question: str) -> Dict[str, Any]:
        ...
        if not hasattr(self, 'qa_chain'):
         ...
         # Single retrieval with automatic token filtering
    ...
    def format_sources(self, sources):
        ...
        for source in sorted_sources:
            ...
            page = source.get('page')  # Get page, returns None if not present
            
            if source_name in unique_sources:
                # Add page if it exists and not already included
                if page is not None and page not in unique_sources[source_name]['pages']:
                    unique_sources[source_name]['pages'].append(page)
            else:
